...
function processPurchase() {
    // Check if the cart array has any items
    if (cart.length === 0) {
        // Display error if cart is empty
        alert("❌ Error: Your cart is empty! Please add products to continue.");
    } else {
        // Display success if cart has items
        alert("✅ Order placed successfully!");

        // Optional: Clear the cart after a successful purchase
        cart = []; 
        saveCart(); // Updates localStorage
